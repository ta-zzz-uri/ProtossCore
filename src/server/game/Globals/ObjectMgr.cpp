...
void ObjectMgr::LoadCreatureCurrencyDrop()
{
	...
	{
		sLog->outString();
		sLog->outString(">>CataChaosCore: Loaded 0 creatures with currency drop.");
		return;
	}
	...
	sLog->outString();
	sLog->outString(">>CataChaosCore: Loaded %lu creatures with currency drop.", (uint64)mCurrencyDropLocaleMap.size());
}
